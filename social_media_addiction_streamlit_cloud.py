"""
Social Media Addiction Analysis - Streamlit Cloud Compatible
===========================================================
...
def load_sample_data():
    ...
    # Create sample dataset similar to the original
    n_students = 705
    ...
    data = {
        ...
        'Academic_Level': np.random.choice(['High School', 'Undergraduate', 'Graduate'], n_students, p=[0.05, 0.6, 0.35]),
        'Country': np.random.choice(['USA', 'India', 'UK', 'Canada', 'Australia', 'Germany', 'France', 'Brazil'], n_students),
        'Avg_Daily_Usage_Hours': np.round(np.random.normal(4.9, 2.0, n_students), 1),
        'Most_Used_Platform': np.random.choice(['Instagram', 'TikTok', 'Snapchat', 'Facebook', 'YouTube', 'Twitter', 'LinkedIn'], n_students, p=[0.3, 0.25, 0.15, 0.1, 0.1, 0.05, 0.05]),
        'Affects_Academic_Performance': np.random.choice([0, 1], n_students, p=[0.36, 0.64]),
        'Sleep_Hours_Per_Night': np.round(np.random.normal(6.5, 1.2, n_students), 1),
        'Mental_Health_Score': np.random.randint(1, 11, n_students),
        'Relationship_Status': np.random.choice(['Single', 'In Relationship', 'Complicated'], n_students),
        'Conflicts_Over_Social_Media': np.random.randint(0, 6, n_students),
        'Addicted_Score': np.random.randint(1, 11, n_students)
    }
    
    df = pd.DataFrame(data)
    
    # Ensure realistic correlations
    # Higher usage should correlate with higher addiction and lower mental health
    for i in range(len(df)):
        usage = df.loc[i, 'Avg_Daily_Usage_Hours']
        if usage > 6:
            df.loc[i, 'Addicted_Score'] = min(10, np.random.randint(7, 11))
            df.loc[i, 'Mental_Health_Score'] = max(1, np.random.randint(1, 6))
        elif usage > 3:
            df.loc[i, 'Addicted_Score'] = np.random.randint(4, 8)
            df.loc[i, 'Mental_Health_Score'] = np.random.randint(4, 8)
        else:
            df.loc[i, 'Addicted_Score'] = min(6, np.random.randint(1, 5))
            df.loc[i, 'Mental_Health_Score'] = max(6, np.random.randint(6, 11))
    
    # Ensure non-negative values
    df['Avg_Daily_Usage_Hours'] = df['Avg_Daily_Usage_Hours'].clip(lower=0.5)
    df['Sleep_Hours_Per_Night'] = df['Sleep_Hours_Per_Night'].clip(lower=3, upper=10)
    
    return df

@st.cache_data
def process_data(df):
    """Process the data and add derived columns."""
...
def main():
    ...
    st.markdown("""
    ### 📊 Dataset Information
    This analysis examines social media addiction patterns among university students.
    The dataset includes usage hours, platform preferences, academic impact, and mental health metrics.
    """)
    ...
    with col2:
        ...
        • Focus on highest risk demographics
        </div>
        """, unsafe_allow_html=True)
